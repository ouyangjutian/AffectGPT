...
def func_return_scores_one(modelname=None, dataset_candidates='affectgpt'):
    ...
    elif dataset_candidates=='emotionllama':
        # 评估Emotion-LLaMA的MER-UniBench全部9个数据集
        # process_datasets = ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]
        process_datasets = ["mer2023"]

    print_per_dataset, avg_score = [], []
    for dataset in process_datasets:
        # process_root = f"output/results-{dataset}/{modelname}"
        process_root = f"output/emotionllama_affectgpt_UniBench/inference_result/results-{dataset}/{modelname}"
        ## 计算指标
...
if __name__ == "__main__":
    ...
    #                 ]:
    for modelname in [
                    'emotionllama_affectgpt_UniBench',
                    ]:                    
        # 评估MER-UniBench全部9个数据集
        # for dataset in ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]:
        for dataset in ["mer2023"]:
            # main_zeroshot_scores(f"output/results-{dataset}/{modelname}")
            main_zeroshot_scores(f"output/emotionllama_affectgpt_UniBench/inference_result/results-{dataset}/{modelname}")
    ...
    for modelname in [
        ...
        print_per_dataset, avg_score = func_return_scores_one(modelname=modelname, dataset_candidates='emotionllama')
